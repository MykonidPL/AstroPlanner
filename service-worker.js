const CACHE='astroplanner-v014-raster-preregression1';
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&k.startsWith('astroplanner-v')&&!k.includes('-rd-')).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
