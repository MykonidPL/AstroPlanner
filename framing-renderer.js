(function(global){
  ...
  function render(layout,opts={}){
    ...
    const panels=(layout.panels||[]).map((p,i)=>{
      ...
      const handle=preview?'':`<path class="frFrameHandle" d="${path}" fill="none" stroke="rgba(0,0,0,0.001)" stroke-width="18" pointer-events="stroke" vector-effect="non-scaling-stroke"/>`;
    ...
    const center=ctr?`<g class="frCenter">${preview?'':`<circle class="frFrameHandle" cx="${ctr.x.toFixed(1)}" cy="${ctr.y.toFixed(1)}" r="16" fill="rgba(0,0,0,0.001)" pointer-events="all"/>`}<circle cx="${ctr.x.toFixed(1)}" cy="${ctr.y.toFixed(1)}" r="${preview?3:4}" fill="#f6c453" pointer-events="none"/></g>`:'';
